...
> 接管验收提示（2026-08-12）：公开 Feed、分类、热门标签、公开主页和待审读取已验证；V5 使用固定分类 ID，在已有库中产生分类语义错位，且种子图片/头像文件不存在，访问会被当前异常处理转换为 500。写状态操作本轮未使用独立验收数据执行，详见 `handoff/backend_status.yaml`。
